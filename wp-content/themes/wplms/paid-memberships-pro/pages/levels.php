...
<tbody>
	...
	<tr class="<?php if($count++ % 2 == 0) { ?>odd<?php } ?><?php if($current_level == $level) { ?> active<?php } ?>">
		...
		<?php if(empty($current_user->membership_level->ID)) { ?>
			<a class="button" href="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>"><?php echo _x('Select', 'Choose a level from levels page', 'pmpro');?></a>
		...
		<?php } elseif($current_level) { ?>      
			...
			<?php
				...
				{
				?>
					<a class="button primary" href="<?php echo pmpro_url("checkout", "?level=" . $level->id, "https")?>"><?php echo _x('Renew', 'Clicking to renew from levels page', 'pmpro');?></a>
				<?php
				}
				else
				{
				?>
					<a class="disabled" href="<?php echo pmpro_url("account")?>"><?php _e('Your&nbsp;Level', 'pmpro');?></a>
				<?php
				}
			?>
