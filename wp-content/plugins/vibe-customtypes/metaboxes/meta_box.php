...
class custom_add_meta_box {
	...
	function admin_enqueue_scripts() {
	        ...
	        wp_enqueue_style( 'timepicker', VIBE_PLUGIN_URL . '/vibe-customtypes/metaboxes/css/timePicker.css' );
	}
			...
			echo '<tr>
					...
					switch( $type ) {
						...
						case 'selectcpt':
                            ...
                            echo '<option value="">'.__('None','vibe-customtypes').'</option>';
                            if(is_array($kposts))
						...
						case 'selectmulticpt': 
							...
							foreach ( $kposts as $kpost ){ 
								if(is_array($meta))
									...
									echo '<option value="' . $kpost->ID . '" '.selected($kpost->ID,$meta).'>' . $kpost->post_title . '</option>';
							}
