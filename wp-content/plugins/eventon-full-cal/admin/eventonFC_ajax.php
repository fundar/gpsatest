...
function evoFC_ajax_days_list(){
	...
	$month_grid = $eventon_fc->get_grid_month(
		$_POST['next_d'],
		$_POST['next_m'], 
		$_POST['next_y'], 
		$start,
...
add_action('wp_ajax_the_ajax_fc', 'evoFC_ajax_days_list');
add_action('wp_ajax_nopriv_the_ajax_fc', 'evoFC_ajax_days_list');
...
function evoFC_ajax_filter($eve_args){
	...
	if(isset($_POST['fc_focus_day'])){
		...
		$number_days_in_month = $eventon_fc->days_in_month( $focused_month_num, $focused_year);
