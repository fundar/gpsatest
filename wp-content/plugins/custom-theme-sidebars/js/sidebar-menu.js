	...
	$.fn.themeSidebarMenu = function() {
		...
		api.initPagination = function( panelLink ) {
			...
			$( panelLink ).on( 'click', function(e) {
					e.preventDefault();
					$.post( ajaxurl, e.target.href.replace(/.*\?/, '').replace(/action=([^&]*)/, '') + '&action=master_sidebar_get_metabox',
						function( resp ) {
		 ...
		 *
		 * AJAX function that enables the user to show/hide
		 * the metaboxes on the options page screen. This 
		 * calls an ajax function server side in order for
		...
		api.initToggles = function() {
			$('.hide-postbox-tog').on( 'click', function(){
				...
				$.post(ajaxurl, {
					...
					page: 'appearance_page_custom_theme_sidebars'
				});
			});
		};
		...
		api.updateQuickSearchResults = function( input ) {
			...
			params = {
				'action': 'master_sidebar_quick_search',
				'response-format': 'markup',
				'menu': $('#menu').val(),
				'master_sidebar_quick_search_nonce': $('#master_sidebar_quick_search_nonce').val(),
				'q': q,
		...
		api.addItemToSidebar = function( menuItem, processMethod, callback ) {
			...
			var nonce     = $( '#master_sidebar_settings_column_nonce' ).val();
			processMethod = processMethod || function(){};
			callback      = callback || function(){};

			var dataObj = {
				'action': 'master_add_sidebar_item',
				'menu': menu,
				'menu-item': menuItem,
				'master_sidebar_settings_column_nonce' : nonce
			};
		...
		api.deleteSidebar = function( sidebarId, processMethod, callback ) {
			...
			processMethod = processMethod || function(){};
			callback      = callback || function(){};
			var nonce     = $( '#master_sidebar_delete_sidebar_instance_nonce' ).val();

			var dataObj = {
				'action': 'master_delete_sidebar_instance',
				'sidebarId': sidebarId,
				'master_sidebar_delete_sidebar_instance_nonce' : nonce
			};
		...
		api.deleteAllSidebars = function( processMethod, callback ) {
			...
			var dataObj = {
					'action': 'master_delete_all_sidebar_instances',
					'master_sidebar_delete_sidebar_instance_nonce' : nonce
			};
		...
		api.createNewSidebar = function( sidebarName, processMethod, callback ) {
			...
			processMethod = processMethod || function(){};
			callback      = callback || function(){};
			var nonce     = $( '#master_sidebar_edit_sidebar_instance_nonce' ).val();

			var dataObj = {
				'action': 'master_create_sidebar_instance',
				'master_sidebar_edit_sidebar_instance_nonce' : nonce,
		...
		api.saveSidebar = function( sidebarName, sidebarId, replacementId, description, processMethod, callback ) {
			...
			$('#sidebar-to-edit li').each( function(e){
				var sidebarDBID           = $(this).find( '.menu-item-data-db-id' ).val();

				//Set position and increment position count
				$(this).find( '.menu-item-data-position' ).attr( 'value', position );
				position++;

				sidebarItems[sidebarDBID] = $(this).getItemData();
			});

			var dataObj = {
				'action': 'master_update_sidebar_instance',
				'sidebarName' : sidebarName,
				'sidebarId': sidebarId,
		...
		api.registerManagementEvents = function() {
			...
			$( '#sidebar-replacements-table .sidebar-delete-link a' ).on( 'click', function(){
				var confirmation = confirm( sidebarsL10n.deleteWarning );
			...
			$( '#sidebar-replacements-table select' ).on( 'change', function(){
				...
				var dataObj = {
					'action': 'master_edit_sidebar_replacement',
					'sidebarId': sidebarId,
					'replacementId' : replacementId,
					'master_sidebar_edit_sidebar_instance_nonce': nonce
				};
